...
import json
'''
    This code calls the API of Google Gmail to extract emails from the mailbox
'''
...
def get_recent_emails_and_save_as_json(service, user_id='me', max_results=5):
    ...
    if not messages:
        ...
        for i, message in enumerate(messages):
            ...
            if email_info:
                file_path = f'../data/email samples/email_{i+1}.json'
                with open(file_path, 'w', encoding='utf-8') as file:
                    json.dump(email_info, file, ensure_ascii=True, indent=4)
